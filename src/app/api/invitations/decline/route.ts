...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const rateLimit = await enforceRateLimit({
      key: rateLimitKeyFromRequest(request, `invite:decline:${actor.userId}`),
      limit: 30,
      windowMs: 60_000
    });
    ...
    const result = await invitationService.decline({ ...body, actorUserId: actor.userId });
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    return toRouteErrorResponse(error, {
      includeValidationError: true,
      includeNotFoundError: true,
      fallbackMessage: 'Failed to decline invitation.'
    });
  }
}
